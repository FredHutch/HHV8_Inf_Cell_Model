...
arms=c(rep("HIV+/KS+",length(both_pos)),rep("HIV-/KS-",length(both_neg)),rep("HIV+/KS-",length(hiv_pos)),rep("HIV-/KS+",length(ks_pos)))
arm_colors=c(rep("red",length(both_pos)),rep("green",length(both_neg)),rep("orange",length(hiv_pos)),rep("blue",length(ks_pos)))
...
ggplot(df, aes(x=x, y=y,color=factor(arm_colors))) +
  geom_smooth(method=lm,color="black") +
  geom_point(color=arm_colors) +
  geom_abline(intercept = 0, slope = 1,color="red", 
                 linetype="dashed")+
